...
{
    NSLog(@"-> JSONLoader is loading: %@", url);
    
    JSONLoader *loadJson = [[self alloc] init];
    loadJson.callbackString = callbackStr;
    loadJson.delegateView = delegate;
    loadJson.urlStr = url;
...
- (void)loadFile
{
    ...
    NSString *dataStr = [[NSString alloc] initWithContentsOfFile:urlStr
