...
-(void)initTimeout
{
    ...
    [self resetCheckTimeout];
    
    [self.closeTimeoutBtn addTarget:self action:@selector(hideTimeout) forControlEvents:UIControlEventTouchUpInside];
}

-(void)resetCheckTimeout
{
    if (checkTimeoutTimer) {
        [checkTimeoutTimer invalidate];
    }
    
    checkTimeoutTimer = [NSTimer scheduledTimerWithTimeInterval: 300.0
                                                         target: self
                                                       selector: @selector(showTimeout)
                                                       userInfo: nil
                                                        repeats: YES];
}
...
-(void)showTimeout
{
    /*CGRect newFrame = self.timeoutPanel.frame;
    newFrame.origin.x = 277 + 20;*/
    
    isTimeOut = YES;
    
    self.timeoutPanel.alpha = 0;
    self.timeoutPanel.hidden = NO;
    
    [UIView animateWithDuration:0.5
                          delay:0.0
                        options: UIViewAnimationOptionCurveEaseOut
                     animations:^{
                         self.timeoutPanel.alpha = 1;
                     }
